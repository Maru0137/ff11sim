...
ff11sim の中核は FFXI のステータス・ダメージ計算である。この式はレベル帯ごとに係数が
切り替わるだけでなく、**0.5 刻みの中間値を保持したまま複数項を合算し、最後に一度だけ
切り捨てる** といった、丸めの位置と順序に敏感な構造を持つ。これを少ない実装コストで
厳密に再現するには、整数と実数を型で区別できる言語が要る。
